...
def func(message):
    ...
    if text == "Добавить_конфиг":
        bot.send_message(
            message.chat.id,
            "Введите имя нового конфига (только латиница, цифры и подчёркивания)",
            reply_markup=types.ReplyKeyboardRemove()
        )
        bot.register_next_step_handler(message, ask_ip_for_new_client)
    elif text == "Конфиги":
        # Ваша логика вывода конфигов
        pass
    else:
...
def ask_ip_for_new_client(message):
    ...
    if not name:
        bot.send_message(
            message.chat.id,
            "Недопустимое имя. Введите заново."
        )
        bot.register_next_step_handler(message, ask_ip_for_new_client)
        return
...
def create_client_with_ip(message):
    ...
    if not check_number_in_range(ip_octet):
        bot.send_message(
            chat_id,
            "Неверный формат IP. Введите число от 2 до 253."
        )
        bot.register_next_step_handler(message, create_client_with_ip)
        return
    ...
    if used:
        bot.send_message(
            chat_id,
            f"IP 10.10.0.{ip_octet} уже используется. Введите другой."
        )
        bot.register_next_step_handler(message, create_client_with_ip)
        return
    ...
    bot.send_message(
        chat_id,
        f"Клиент {username} с IP 10.10.0.{ip_octet} создан."
    )
